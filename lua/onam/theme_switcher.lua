...
function M.next_theme()
	local index = M.get_index() + 1
	if index > #M.themes then
		index = 1
	end
	M.set_theme(M.themes[index])
	print("Next theme: " .. M.themes[index])
end
...
function M.previous_theme()
	...
	M.set_theme(M.themes[index])
	print("Previous theme: " .. M.themes[index])
end

function M.add_candidate()
	local theme = M.get_index()
	table.insert(M.canidates, M.themes[theme])
	print("Added: " .. M.themes[theme])
end

function M.get_canidates()
	if #M.canidates == 0 then
		return "No canidates"
	end
	return M.canidates
end
