...
local function update_hover_content()
	...
	-- local line_count = vim.api.nvim_buf_line_count(orig_buf)
	-- local current_line = vim.api.nvim_buf_get_lines(orig_buf, cursor_pos[1] - 1, cursor_pos[1], false)[1] or ""
...
local function create_hover_split(command, remain_focused)
	...
	vim.lsp.buf_request_all(orig_buf, "textDocument/hover", vim.lsp.util.make_position_params(), function(err, result)
...
function M.setup(options)
	...
	-- vim.cmd([[ autocmd CursorMoved,CursorMovedI * lua require('onam.helpers.lsp.hover').update_hover_content() ]])
end
