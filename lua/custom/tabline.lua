  ...
  __call = function()
    ...
    if not vim.g._tabline_name_restored then
      for tabnr, tabid in ipairs(vim.api.nvim_list_tabpages()) do
        ...
        if not tabname_id and tabname_nr then vim.t[tabid]._tabname = tabname_nr end
      end
    end
    for tabnr, tabid in ipairs(vim.api.nvim_list_tabpages()) do
      ...
      if vim.g._tabline_name_restored then vim.g["Tabname" .. tabnr] = vim.t[tabid]._tabname end
      table.insert(
        tabnames,
        stl_hl(
          string.format(
...
function _G._tabline.rename(tabid, name)
  if not tabid or not vim.api.nvim_tabpage_is_valid(tabid) then return end
...
vim.api.nvim_create_autocmd("TabClosed", {
  ...
  group = groupid,
  callback = function(info) vim.g["Tabname" .. info.file] = nil end,
})
