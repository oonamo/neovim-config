...
local function signature_handler(_, result, ctx)
  ...
  if result == nil or result.signatures == nil or result.signatures[1] == nil then return end
  ...
  if actSig.documentation and actSig.documentation.value then
    ...
    local lines, _ = vim.lsp.util.convert_signature_help_to_markdown_lines(result, vim.bo[ctx.bufnr].filetype, triggers)
...
local function echo_handler(_, result, ctx)
  ...
  if actSig.label ~= "" then
    local label = vim
      .iter(ipairs(actSig.parameters))
      :map(function(i, parameter)
        local hl = "Normal"
        if actSig.activeParameter and i == actSig.activeParameter then hl = "Special" end
