...
--================== Find ====================
if not Config.plugs.snacks then
  ...
  map_leader("n", "fv", "<Cmd>Pick files<CR>", "Files")
...
local git_diff_cmd = [[<cmd>Git diff --cached --patch-with-stat<cr>]]
...
map_leader("n", "gf", "<cmd>Git add %<cr>", "Stage current file")
map_leader("", "gh", "<cmd>lua MiniGit.show_range_history()<cr>", "Git Range History")
map_leader("n", "gC", function()
  vim.cmd("Git diff --cached --patch-with-stat")
  vim.cmd("horizontal Git commit --amend")
end, "Ammend Commit with diff")
map_leader("n", "gG", "<Cmd>Git status<CR>", "Status")
map_leader("n", "gd", "<Cmd>Git diff --patch-with-stat<CR>", "Diff")
map_leader("n", "gD", "<Cmd>Git diff --patch-with-stat -- %<CR>", "Diff buffer")
...
--   "n",
--   "e",
--   function() require("oil").open(vim.fn.fnamemodify(vim.api.nvim_buf_get_name(0), ":p:h")) end,
--   "File directory"
-- )
...
map_leader("n", "ot", "<cmd>lua MiniTrailspace.trim()<cr>", "Trim Whitespace")
